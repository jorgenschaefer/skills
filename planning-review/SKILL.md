...
- **Are cross-cutting concerns ticketed?** Read [cross-cutting-concerns.md](cross-cutting-concerns.md) for the canonical list. For each concern, check whether the design doc addressed it architecturally, then verify a ticket exists in the backlog covering it. Concerns most commonly left unticketed: observability setup, migration scripts, runbook updates, monitoring/alerting configuration.
