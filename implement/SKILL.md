...
- **Say where the ticket stands.** A reviewer meets the ticket mid-run: its `## Record` and its `status: done` are written in `## Finish`, after both reviews return, so `status` still reads `todo` and nothing is yet written down about what this build proved, decided or left open. Put that in the dispatch as the fact it is, and stop there. What follows from it is the reviewer's to draw, and adding it is the pre-judging the bullet above forbids.
- **Hand over the proof list.** `### Prove the contract before the reviews see it` leaves you with a statement per `Satisfies` id - or per behaviour the diff touches, where the ticket claims none - naming the test that fails without that behaviour and carrying both of the proofs that step ran on it, the deletion and the moved edge. Both, because the code review's coverage bullet below asks whether a test fails when the behaviour *changes*, and the deletion alone does not answer that. Both dispatches name the list among their inputs. Compose it as you dispatch rather than carrying one from earlier, so a re-dispatch over review fixes shows the tests those fixes added or changed. A test any review fix wrote or changed goes through "Break it twice" before it enters any list, the first dispatch as much as the second, since the RED-first loop a fix follows proves the deletion and never the edge. It reports checks you ran rather than conclusions you reached, and that is also its limit - a reviewer cannot re-run them, so a challenge to it is answered by naming what failed and where, never by restating that it did.
