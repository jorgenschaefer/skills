...
description: Use after /discovery (and optional /discovery-increment) when a spec is ready to implement but still leaves technical implementation decisions an implementer would otherwise pick arbitrarily. Optional depth pass, distinct from /discovery-design (which covers UI). Triggers on "/discovery-implementation", "nail down the implementation", "what would an implementer have to guess", or before /implement on a non-trivial spec.
---

# Discovery Implementation

Given a path to a `/discovery`-shaped spec (the whole discovery doc or a single `INCREMENT-NN.md` - any filename is fine), walk it as if implementing it TDD, top to bottom, and surface every point where an implementer would otherwise just pick something. Resolve those decisions and enrich the spec in place, so `/implement` builds it as intended instead of inventing the technical approach. This pass covers how the feature is built; its sibling `/discovery-design` covers how it looks.
...
- Add an **Implementation decisions** section: each decision, its resolution, and an inline `_Why: ..._` when a wrong turn was the risk.
