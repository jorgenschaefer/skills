...
Most of these skills compose into one flow with a single front door and a single implementer.
...
A feature - one change worth shipping on its own:
  ...
  /discovery ──→ SPEC.md ──→ /spec-to-tickets ──→ tickets/ ──→ ./loop.sh
     ...
     ┌─────────────────────────────────────────────────────────────┘
     │
     │  ┌─ /implement-ticket × N  unattended
     ├──┤  /check-against-spec   gaps → tickets
     │  └─ /critique             blockers → tickets
     │     (the check loops; the review reads once, then what it filed)
     │
     └─ /handover                the PR description; then ./accept.sh

A small change or bugfix:

  /discovery ──→ one ticket ──→ /implement
```

`loop.sh` runs unattended for hours, so it prints only each step's narration - one line per phase - plus position, duration and cost. The exception is the closing handover, which is printed whole: it is the description the whole run was for. Every run ends in one of three states - clean, halted, or standing on something a human must rule on - and each prints the same two things: the driver's own account, collected from the tickets rather than judged, and the handover's. A clean run ends by naming the one command that accepts it, `./accept.sh`, which deletes the spec and the tickets in a single commit; that one is yours to run, since it is yours to authorise. Every step's full JSON transcript is kept under a log directory it names at startup, so a halt tells you exactly which file to open. A session that dies does not end a run: running out of usage is waited out - the stream names the second the window reopens, and `MAX_WAIT_HOURS` caps how long that may take - and any other death is tried again after a pause, seven of them over about three hours (`RETRY_DELAYS`). Only an agent's own halt, a queue it has no path through, a workflow test changed without authorisation, or an error no delay can fix stops the loop for a human. The reviews run on a different model from the one that built the code - `BUILD_MODEL` and `REVIEW_MODEL`, which the driver refuses to start with set to the same thing, since a model reviewing its own work shares its own blind spots. Nobody is there to approve a tool call either, so it needs standing permission for the edits, commands and commits a ticket makes: `claude -p` cannot prompt, and what it cannot get approved it declines. The driver has tests of its own in `tests/run.sh`, plain bash against transcripts recorded from real runs.

The split test routes between the two lanes, and `/discovery` applies it: *could you ship this on its own, and would that be worth shipping as an improvement in itself?* Yes means a feature; no, but someone outside the code can see the difference, means a change and one ticket; no difference at all means it doesn't want a ticket.

The ticket is the unit both lanes produce and the only thing `/implement` builds. A run has nobody to ask, which is what `/implement-ticket` tells it: so it never guesses - it halts, records why in the ticket, and the loop stops for a human. Everything requiring code is a ticket, including what the end-of-run reviews find: `/check-against-spec` files its gaps, and the loop turns `/critique`'s blockers into tickets too, so a late fix is built and re-verified rather than patched in behind the checks. `/critique` itself stays a generic review skill - the caller knows about tickets, the reviewer doesn't. Spec and tickets are deleted once the work is accepted, by `./accept.sh` in a single commit that refuses to run unless every ticket is done and the tree is clean; git history keeps them, and anything that must outlive the run is promoted into the glossary, a comment, or an ADR first. What each step hands to a human - the decomposition's hand-off, the run's closing brief - is presented inline and never written down: it is read once, at the moment someone decides, and a copy on disk only outlives the decision.

## Available skills

- **check-against-spec** - check a finished feature against the spec it was built from: every criterion met *and* pinned by a test that would fail without it, every constraint verified, every non-goal respected, and nothing built that traces to nothing; uses the project's mutation testing tool where one is configured, since that claim is the one worth executing rather than judging; files a ticket for each gap so it re-enters the same loop
...
- **handover** - close out a run by writing the pull request description for it: what the branch does now, how it works, what a reviewer should look at first, and what is still uncertain; proposes the rationale worth promoting into a comment or the glossary before `./accept.sh` deletes the paper
