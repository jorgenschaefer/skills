...
- **Adapter boundaries respected.** Keep business logic isolated from both inbound and outbound adapters. The inbound adapter (route handler, controller) authenticates, validates input into a domain object, calls business logic, and maps the result to an HTTP response — no domain rules. The business logic operates on domain objects only — no HTTP types, no validation-schema types (e.g., Zod inferred types), no ORM entities or DB row types. The outbound adapter (repository) translates domain objects to DB format and back — no domain rules. For simple CRUD this can all live in one function, but the three concerns must be visibly distinct and not tangled. Types follow the same boundary: validation schemas live only in the inbound adapter, domain types live with the business logic, database types live only in the outbound adapter.
