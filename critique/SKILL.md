...
### Correctness
- All tests pass.
- Every piece of business logic is pinned by a test: removing or changing the code would make a test fail. Gaps in that safety net are findings.
- External adapters — the thin edge that talks to a third-party SDK, the network, or IO — may be untested when they genuinely can't be unit-tested. The business logic behind them must be fully tested.
...
- **Business logic talks to external systems through intention-revealing functions, not raw framework calls.** It calls domain-named queries like `getActiveFoo()` or `getFooByCompanyId()` — the wrapper names the intent in domain terms and keeps framework/ORM detail out of the logic. A passthrough that just relays a framework query object (`getFoo(prismaWhereClause)` → `prisma.foo.findMany(...)`) does not count: it leaks the composable ORM query through a thin disguise. Ideally the wrapper returns a domain type rather than a framework type (the same shape is fine). This is about abstraction, not dependency injection — don't expect injected dependencies, and don't over-abstract: a named query returning a domain type is enough (YAGNI).
...
Report findings to the conversation, grouped by severity:
