...
description: Use this skill to implement a single ticket using TDD (test-driven development). Trigger when the user says "implement ticket #X", "TDD this", or hands you a ticket file and asks you to build it. Also invoked as a subagent by the implementation orchestrator. The output is working, tested, committed code satisfying every acceptance criterion.
---

# Implementation: TDD

The goal of this skill is to take a single ticket and produce working, tested, reviewable code that satisfies its acceptance criteria. The output is code, tests, and a description of what was done — all within the scope the ticket defines.
...
1. **The ticket.** A specific ticket file with goal, scope, and acceptance criteria. If the user gestures at "the next ticket" without specifying, ask which one. Once identified, immediately update its **Status** to `In Progress` before doing anything else. If a review file was provided in the invocation prompt, this is **review response mode** — read §Review response mode before proceeding.
...
6. **The reference files for this skill.** Read [tests.md](tests.md), [deep-modules.md](deep-modules.md), [mocking.md](mocking.md), [refactoring.md](refactoring.md), [architecture-principles.md](architecture-principles.md), and [code-style.md](code-style.md) before writing any code.
...
**When invoked as a subagent** (via the `Agent` tool), you cannot interact with the user. If you encounter a blocker — an ambiguous acceptance criterion, a genuine conflict with the design, or unexpected state that suggests a wrong assumption — stop immediately and output:

**BLOCKED:** [issue in one sentence]
**Options:**
1. [option A]
2. [option B]

Return without proceeding. The orchestrator will surface this to the user.

**When invoked directly**, stop and ask before proceeding when:
...
- **Correctness, security, observability, performance.** Read [code-style.md](code-style.md) §Quality and address these while writing.
...
- **Things you noticed but did NOT fix.** List these as incidental findings (file path + description). When running directly (not as a subagent), use the `Agent` tool with `subagent_type: "general-purpose"` and this prompt:
  ...
  After the subagent finishes, list the ticket numbers it created here — not free-form observations. When running as a subagent, list the findings plainly in your output; the orchestrator will handle boy-scout.
...
When invoked with a review file (e.g., "address findings in `<path>`"), read that file first. Work through each finding in order:

- **Blocker**: fix it, run tests, commit
- **Should-fix**: address it, commit
- **Nit**: use judgment

After all findings are addressed, run through the full §Things to verify before declaring done checklist to confirm the codebase is clean. Then produce the normal output (PR description + commit hashes).

## After implementation

When the TDD loop is complete and the checklist is done, end your response with:

1. A PR description (see §Producing the PR description)
2. A clearly labeled list of every commit hash: **Commit hashes:** `<hash1>`, `<hash2>`, ...
3. Any incidental findings not yet handled (for the orchestrator to pass to boy-scout if needed)
